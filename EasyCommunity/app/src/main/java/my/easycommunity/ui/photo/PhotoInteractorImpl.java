...
public class PhotoInteractorImpl implements PhotoInteractor {
    ...
    private List<GankPhoto.ResultsBean> list ;
    ...
    private void netData(final int page , Observable.Transformer transformer)
    {
        ...
        compositeSubscription =service.getPhoto(page)
                ...
                {
                    @Override
                    public void call(GankPhoto gankPhoto)
                    {
                        if(gankPhoto.isError() == false && gankPhoto.getResults() != null)
                        {
                            ...
                            if(page==1){
                                ...
                                Observable.from(gankPhoto.getResults()).subscribe(new Action1<GankPhoto.ResultsBean>()
                                {
                                    @Override
